...
nocache("../XeonCheems13.js", (module) =>
  console.log(
    color("[ CHANGE ]", "green"),
    color(`'${module}'`, "green"),
    "Updated"
    ...
    if (!!phoneNumber) {
      ...
      if (
        ...
        console.log(
          chalk.bgBlack(
            chalk.redBright(
              "Start with country code of your WhatsApp Number, Example : +6283854436555"
            )
          )
        );
        process.exit(0);
  ...
  XeonBotInc.ev.on("connection.update", async (update) => {
    ...
    try {
      ...
      if (
        ...
        console.log(color(`\n${themeemoji} YT CHANNEL: DearlyPebriano`, "magenta"));
        console.log(color(`${themeemoji} GITHUB: dearlypebriano `, "magenta"));
        console.log(
          color(`${themeemoji} INSTAGRAM: @drlyfebriano `, "magenta")
        );
...
└┬❖ 「 @${xeonName.split("@")[0]}  」
      ...
      for (let XeonFucks of XeonPapa) {
        if (XeonFucks.isGroup == false) {
          if (XeonFucks.status == "offer") {
            let XeonBlokMsg = await XeonBotInc.sendTextWithMentions(
              XeonFucks.from,
              `*${XeonBotInc.user.name}* can't receive ${
  ...
  XeonBotInc.sendFile = async (
    ...
    try {
      ...
      if (!m)
        m = await XeonBotInc.sendMessage(
          jid,
          { ...message, [mtype]: file },
          { ...opt, ...options }
        );
